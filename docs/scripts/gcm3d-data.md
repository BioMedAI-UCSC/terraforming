...
These scripts stage the external datasets the 3-D GCM (`src.gcm3d`) needs and
...
| `stage_ames_co2_radiation.py` | Decode the bundled Ames 12-band Fortran CO₂ k-tables into a compact JAX `.npz` asset (nearly pure-CO₂ mixture, H₂O mass fraction 1e-7). | `package/src/gcm3d/ames_co2_12band.npz` (bundled; consumed by `ames_radiation.py`). |
