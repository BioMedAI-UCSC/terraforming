...
def active_params(params, ablation):
    disabled = {"tendency-only": ("encoder", "decoder"), "adapters-only": ("physics",)}.get(ablation, ())
...
def main():
    ...
    p.add_argument("--ablation", choices=("full", "tendency-only", "adapters-only"), default="full")
...
def run(args, cache, devices):
    ...
    evaluate = jax.jit(lambda p, x: e.metrics(active_params(p, args.ablation), x, stats, horizon))
    ...
    while completed < args.steps:
        ...
        for ds in cache.iterate([selections[i] for i in order], "train"):
            size = ds.sizes["time"] - args.spinup - reserved_horizon
