...
class InvalidNativeWindow(ValueError):
    """Source ordering makes this window unsuitable for a native forecast."""
...
def validate(ds, split, *, prepared=False):
    ...
    spacing = np.diff(np.asarray(ds.time, dtype=np.float64))
    bad = np.flatnonzero(~np.isclose(spacing, 1 / 12, rtol=0, atol=1e-9))
    if len(bad):
        i = int(bad[0])
        raise InvalidNativeWindow(
            f"non-contiguous native MACDA time: indices {i}->{i+1}, "
            f"sols {float(ds.time[i]):.12g}->{float(ds.time[i+1]):.12g}, "
            f"delta={spacing[i]:.12g}, expected={1/12:.12g}")
    if not np.isin(ds.MY_Ls, SPLITS[split]).all():
        raise InvalidNativeWindow(f"chunk crosses the requested split: {split}, years={np.unique(ds.MY_Ls).tolist()}")
...
class NativeCache:
    ...
    def iterate(self, selections, split, *, indexed=False):
        """Skip unusable source windows, but preserve their original cursor slots.

        Indexed training iteration yields (window, None) for exclusions. Other
        callers receive only valid datasets. I/O, checksum and field errors
        still fail loudly; only temporal/split boundaries are excluded.
        """
        ...
        with ThreadPoolExecutor(max_workers=1) as pool:
            ...
            while pending is not None:
                try:
                    ds = pending.result()
                except InvalidNativeWindow as error:
                    event("chunk_excluded", split=split, window=current, reason=str(error))
                    ds = None
